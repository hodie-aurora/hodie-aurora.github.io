...
3. **Controller Manager**：
   ...
   - **调整 ReplicaSet**：ReplicaSet 控制器根据新的副本数量配置，更新 ReplicaSet 对象的期望状态，从 1 个副本调整为 2 个（更新期望状态都要经过API Server之后发送到etcd）。
4. **ReplicaSet 控制器**：

   - **创建新 Pod**：ReplicaSet 控制器检测到需要增加副本数量时，通过 API Server 创建新的 Pod 的配置文件并同步到 etcd。
5. **Scheduler**：

   - **分配节点**：Scheduler 接收到 API Server 的新 Pod 创建请求后，会经过预选和优选挑出合适的节点，并将调度结果通过 API Server 写入 etcd。
   - 通知状态：API Server 通知 Kubelet 创建 Pod
6. **Kubelet**：
