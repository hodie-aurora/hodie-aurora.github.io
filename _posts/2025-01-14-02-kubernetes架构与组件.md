...
这个请求首先发送到API Server，API Server 经过认证和授权检查之后，将请求持久化到 etcd，并API Server 在更新 etcd 后，会触发事件通知机制，相关控制器（例如 ReplicaSet 控制器）会监听这些事件，并根据最新的状态进行调整。
...
API Server 接收到获取集群内某些组件信息的指令时，通过查询 etcd 获取各个节点中组件的最新状态信息。比如，当您使用以下命令：
...
#### Scheduler 调度 Pod 过程举例

当用户通过 `kubectl` 命令创建一个新的 Pod 时，Pod 的调度过程大致可以分为以下几个步骤：

1. **Pod 创建** ：用户通过 `kubectl` 提交创建 Pod 的请求，API Server 接收到请求后，将 Pod 的信息存储在 etcd 中，标记为待调度状态。
2. **Scheduler 监听** ：Scheduler 通过 Watch API 持续监听未调度的 Pod 资源对象的变化事件，当检测到新创建的未调度 Pod 时，进行调度。
3. **预选阶段** ：Scheduler 对集群中的节点进行筛选，过滤掉不符合基本要求的节点，例如资源不足或状态不可用的节点。
4. **优选阶段** ：Scheduler 对预选出的节点进行打分，根据节点的资源剩余情况、网络拓扑、历史性能数据等综合考量，选择最佳节点。
5. **调度绑定** ：Scheduler 将 Pod 绑定到选定的节点，并将调度结果更新到 API Server。
6. **Pod 运行** ：节点上的 kubelet 组件监控到新的 Pod 调度到本节点，开始拉取镜像并启动容器，最终使 Pod 进入运行状态。
...
3. **Controller Manager**：
   ...
   - **监控变更** ：ReplicaSet 控制器通过 API Server 订阅和监听 Deployment 和 ReplicaSet 对象的变更事件。当 API Server 检测到 Deployment 的副本数量发生变化时，会通知 ReplicaSet 控制器。ReplicaSet 控制器再通过 API Server 获取最新的副本数量配置。
   - **调整 ReplicaSet**：ReplicaSet 控制器根据新的副本数量配置，更新 ReplicaSet 对象的期望状态（从 1 个副本调整为 2 个）。
     **PS**:更新期望状态并不代表创建pod，只是告知api server去创建pod，而api server创建pod的配置文件并同步到etcd标记为未调度，scheduler去调度这个pod这个pod才真正的被创建
4. **ReplicaSet 控制器**：

   - **创建新 Pod**：ReplicaSet 控制器检测到需要增加副本数量时，通过 API Server 创建新的 Pod。
5. **Scheduler**：

   - **分配节点**：Scheduler 接收到新 Pod 创建请求后，会将新创建的 Pod 分配到合适的节点上运行。
6. **Kubelet**：

   - **启动 Pod**：Kubelet 监控到节点上的新 Pod 请求，下载 Pod 的镜像并启动容器。
   - **报告状态**：Kubelet 将新 Pod 的状态报告给 API Server。
7. **API Server 和 Controller Manager**：
