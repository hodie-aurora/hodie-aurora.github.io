...
这个请求首先发送到API Server，API Server 经过认证和授权检查之后，将请求持久化到 etcd，并通过事件通知机制通知 Controller Manager 组件，由 Controller Manager 组件调度 ReplicaSet 控制器去调整 Deployment 中 Pod 的数量
...
2. **Scheduler 监听** ：Scheduler 持续监听 API Server 的事件，捕捉到新创建的未调度 Pod 事件。
...
4. **ReplicaSet 控制器**：
   ...
   - **创建新 Pod**：ReplicaSet 控制器通过 API Server 发送请求，创建新的 Pod 以满足新的副本数量要求。
5. **Scheduler**：
