...
* **链路追踪**
  ...
  当一条请求进入集群时，当这条请求发送到服务A时，如果它没有traceid，则skywalking为其生成traceid，经过skywalking的自动插桩，skywalking将记录traceid、请求路径、时间、请求载荷，span信息等等信息并生成sw8头放入请求，这个请求在此应用内的每个方法都会被记录sw8头，并且生成的spanid指向父id，在请求出服务时skywalking agent span信息异步发送给skywalking oap，skywalking oap将信息存入es
