...
全局变量 + sync/atomic（手动实现）_posts/2025-10-26-01-AAll.md
...
双statefulset prometheus + 双 sidecar thanos sidecar
statefulset thanos compactor 通过启动命令行参数配置降采样和保留策略
statefulset thanos Store Gateway
deploy thanos Querier

采集流程：每个statefulset prometheus对采集数据进行replcas打标签，并且将固定时长，比如2小时的数据块保留在本地磁盘，thanis sidecar 检测本地磁盘如果发现数据块则上传至minio中，（本地的数据会根据prometheus自己的规则进行保留）Thanos compactor通过配置的采样策略，将minio中的数据按照规则降采样和保留
查询流程：通过定制页面访问thanos querier接口，thanos querier去同时访问两个prometheus 并且访问 thanos gateway，访问 prometheus的部分是热数据，计算哈希值如果相同直接使用优先级更高的标签来源（将标签靠前的作为主本标签靠后的作为副本进行合并去重），如果不同则都会被展示（比如prometheus A发现了某服务而B没有），访问thanos store gateway 的部分，thanos store gateway会实现缓存minio 索引，直接拉取数据块
